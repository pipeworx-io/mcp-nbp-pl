...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/exchange_rate_table \
  -H 'Content-Type: application/json' \
  -d '{"table":"A"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/exchange_rate_table`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
